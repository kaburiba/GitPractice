...
/**
 * Servlet implementation class KadaiLoginSccess
 */
@WebServlet("/KadaiLoginSccess")
public class KadaiLoginSccess extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public KadaiLoginSccess() {
        super();
...
request.getParameter("UTF-8");
		...
		System.out.println("ログイン字のソルト：+salt");
		// 入力されたID、ハッシュしたPWに一致するユーザを検索する
		KadaiAccount account = AccountDAO.login(mail, hashedPw);
		...
		if(account == null) {
			...
			String view = "WEB-INF/view/Kadai-sccess-login.jsp";
			RequestDispatcher dispatcher = request.getRequestDispatcher(view);
			dispatcher.forward(request, response);
		}
	}
